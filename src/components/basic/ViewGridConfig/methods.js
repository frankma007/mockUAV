...
let methods = {
  ...
  saveAudit () { // 保存审核
    ...
    let url = this.getUrl(this.const.AUDIT)
    let auditParams = { id: keys, auditStatus: this.auditParam.status, auditReason: this.auditParam.reason }
    this.http.post(url, auditParams, '审核中....').then(x => {
  ...
  initDicKeys () { // 初始化字典数据
    ...
    this.http.post(baseApi + '/api/Sys_Dictionary/GetVueDictionary', keys).then(dic => {
